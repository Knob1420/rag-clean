...
用法：
    ...
    # 跳过已有中间结果（断点续传）
    python batch_import.py --skip-existing
...
def main():
    ...
    parser.add_argument("--skip-existing", action="store_true", help="跳过已有中间结果的文件")
    ...
    print(f"  模式: {'dry-run' if args.dry_run else ('index-only' if args.index_only else 'full')}")
    print(f"  发现文件: {len(files)}")
    print(f"{'='*70}\n")

    if args.dry_run:
        for i, (f, fmt, dataset_id) in enumerate(files):
            rel = f.relative_to(RAW_DIR / dataset_id) if (RAW_DIR / dataset_id) in f.parents else f
    ...
    for i, (f, fmt, dataset_id) in enumerate(files):
        ...
        try:
            documents = process_document(
                file_path=str(f),
                dataset_id=dataset_id,
                load_intermediate=args.index_only,
                processed_dir=PROCESSED_DIR,
            )
            print(f"  处理完成: {len(documents)} documents")
            success += 1
            results.append({
                "dataset_id": dataset_id,
                "title": f.stem,
    ...
    # 按 dataset_id 统计
    dataset_counts = Counter(r.get("dataset_id") for r in results if r.get("status") == "ok")
    if dataset_counts:
        print(f"\n  按数据集分布:")
