...
class RetrievalService:
    ...
    def search(
        ...
        Args:
            query_string: 最终的 Lucene query_string（由调用方构建）
            query_vector: 最终的 embedding 向量（由调用方构建，含 HyDE 等）
            options: 检索选项
    ...
    def _vector_search(
        ...
        Args:
            query_vector: 最终的 embedding 向量（由调用方构建）
        """
    ...
    def _rerank(
        ...
        Args:
            query: 原始用户 query（用于构建增强 rerank query）
            chunks: 待 rerank 的候选结果
            options: 检索选项，需要 options.rerank_top_k
        """
        if not chunks:
            return chunks
        ...
        top_k = options.rerank_top_k if options.rerank_top_k else options.top_k
        logger.info(f"Rerank 重排序: {len(chunks)} 个候选结果 → 返回 top {top_k}")
    ...
    def _build_bm25_query(
        self,
        query: str,
        _options: RetrievalOptions = None,
    ) -> str:
        """
